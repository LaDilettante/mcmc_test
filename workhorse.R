...
model <- function(theta_prior, sigma2_prior) {
  ...
  cond_theta <- function(state, y) {
    ...
    tau2_n <- 1 / (1 / theta_prior$get("var") + n / state$get("sigma2"))
    mu_n <- (theta_prior$get("mean") / theta_prior$get("var") + n * ybar / state$get("sigma2")) / 
              (1 / theta_prior$get("var") + n / state$get("sigma2"))
...
state <- function(theta, sigma2) {
  ...
  get <- function(param = c("theta", "sigma2")) {
    param <- match.arg(param)
    switch(param,
           theta = theta,
           sigma2 = sigma2)
  ...
  set_theta <- function(value) theta <<- value
  set_sigma2 <- function(value) sigma2 <<- value
  
  return(list(get = get, 
              set_theta = set_theta, 
              set_sigma2 = set_sigma2))
}